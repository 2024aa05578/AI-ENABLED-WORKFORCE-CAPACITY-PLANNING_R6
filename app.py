...
with st.sidebar.form("planning_assumptions_form"):
    ...
    for region in REGIONS:
        ...
        with col1:
            bau_value = st.number_input(
                ...
                step=1.0,
                key=f"{clean_key(region)}_bau_input"
            )
        ...
        with col2:
            dc_value = st.number_input(
                ...
                step=1.0,
                key=f"{clean_key(region)}_dc_input"
            )

        new_regional_growth[region] = {
            "BAU": bau_value,
            "DC": dc_value
        }
    ...
    for product in PRODUCTS:
        new_attrition[product] = st.number_input(
            f"{product} Attrition %",
            min_value=0.0,
            max_value=30.0,
            value=float(st.session_state.attrition_parameters[product]),
            step=0.5,
            key=f"{clean_key(product)}_attrition_input"
        )

    st.markdown("---")
    st.subheader("Workforce Productivity")

    new_productive_hours = st.number_input(
    ...
    new_target_utilization = st.number_input(
    ...
    apply_button = st.form_submit_button("Apply Assumptions")

if apply_button:
    st.session_state.regional_growth = new_regional_growth
    st.session_state.attrition_parameters = new_attrition
    st.session_state.productive_hours = new_productive_hours
    st.session_state.working_days = new_working_days
    st.session_state.target_utilization = new_target_utilization
...
st.info(
    ...
    "click Apply Assumptions, and review BAU, DC and combined hiring requirement."
)
